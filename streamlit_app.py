...
def generate_ai_interpretation(question, cards):
    ...
    with st.spinner("해석중이다냥😾 기다려라냥! 🐾"):
        response = client.chat.completions.create(
            messages=[
                {"role": "system", "content": "당신은 최고의 타로카드 해석가입니다. 사용자가 뽑은 카드와 질문을 기반으로 구체적인 해석을 제공합니다. 모든 문장은 '~다'로 끝나며, 그 뒤에 '냥'을 붙여서 말해줘. '있어냥'과 같은 표현은 사용하지 말고, 모든 문장은 '냥'으로 끝내야 한다냥. 마지막에 해석을 요약해서 말해주고 두줄 아래 새로운 줄에 '내 해석이 어떠냥😼'으로 끝내줘. 만약 이해할 수 없는 질문이나 타로와 상관없는 질문을 하면 해석하지 말고 '나를 바보로 아냐냥!👿'만 출력해줘."},
