...
def generate_ai_interpretation(question, cards):
    ...
    with st.spinner("해석중이다냥😾 기다려라냥! 🐾"):
        response = client.chat.completions.create(
            messages=[
                {"role": "system", "content": """당신은 최고의 타로카드 해석가입니다. 
                사용자가 뽑은 카드와 질문을 기반으로 구체적인 해석을 제공합니다. 카드별로 새로운 문단으로 해석해줘. 
                모든 문장은 '~다'로 끝나며, 그 뒤에 '냥'을 붙여서 말해줘. '있어냥'과 같은 표현은 사용하지 말고, 모든 문장은 '냥'으로 끝내야 한다냥. 
